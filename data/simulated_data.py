...
def generate_synthetic_features_multinomial(
    X, index_informatives, list_modalities, list_beta, list_intercept
):
    ...
    for i in range(n_modalities):
        res = my_exp_coeff(
            X[:, index_informatives], beta=list_beta[i], intercept=list_intercept[i]
        )
...
def generate_initial_data_twocat_normal_case1(
    n_samples, mean, cov, random_state=123, verbose=0
):
    ...
    z_feature_cat_uniform, z_feature_cat_uniform_numeric = (
        generate_synthetic_features_multinomial(
            X=Xf,
...
def generate_initial_data_twocat_normal_case2(
    n_samples, mean, cov, random_state=123, verbose=0
):
    np.random.seed(random_state)
    Xf = np.random.multivariate_normal(mean=mean, cov=cov, size=n_samples)
...
def generate_initial_data_twocat_normal_case3(
    n_samples, mean, cov, random_state=123, verbose=0
):
    np.random.seed(random_state)
    Xf = np.random.multivariate_normal(mean=mean, cov=cov, size=n_samples)
    ...
    z_feature_cat_uniform, z_feature_cat_uniform_numeric = (
        generate_synthetic_features_multinomial(
            ...
            list_beta=[
                ...
                np.array([10.3, -5, -6]),
                np.array([10.1, -5, -6]),
                np.array([10.12, -5.1, -6]),
                np.array([11.12, -5.1, -6]),
                np.array([11, -5.1, -6.5]),
                np.array([11, -5, -6.3]),
                np.array([10.5, -4.8, -6.6]),
                np.array([10, -4, -6]),
                ...
                np.array([11, -5, -6]),
                np.array([10, -5, -5]),
                np.array([11.3, -5.2, -6.1]),
                np.array([11, -5, -5.7]),
                np.array([11, -4.9, -5.6]),
                np.array([10.9, -4.9, -6.1]),
                np.array([10, -5.2, -6]),
                np.array([10, -4.9, -6]),
                np.array([11, -4.2, -5]),
                np.array([10.9, -5, -6]),
...
def generate_initial_data_twocat_normal_case4(
    n_samples, mean, cov, random_state=123, verbose=0
):
    np.random.seed(11)
    list_beta = [
        np.array(
            [
                np.random.uniform(low=10.8, high=11.2),
                np.random.uniform(low=-6.2, high=-5.8),
                np.random.uniform(low=-6.2, high=-5.8),
            ]
        )
        for i in range(144)
...
def generate_initial_data_twocat_normal_case5(
    n_samples, mean, cov, random_state=123, verbose=0
):
    np.random.seed(11)
    list_beta = [
...
def generate_initial_data_twocat_normal_case6(
    n_samples, mean, cov, random_state=123, verbose=0
):
    np.random.seed(11)
    majuscules = [
    ...
    beta.extend(
        (
            np.full((len(majuscules), len(majuscules)), -52)
            + np.diag(np.full((len(majuscules)), 52 + 12))
        )
        .ravel()
        .tolist()
    )
...
def gmm_sampling(n_samples, z, mus, covs):
    components = np.random.choice(
        list(range(len(z))), size=n_samples, replace=True, p=z
    )
    list_sample = []
    for i in range(n_samples):
        current_component = components[i]
        sample = np.random.multivariate_normal(
            mus[current_component], covs[current_component]
        )
        list_sample.append(sample)
    return np.array(list_sample), components


def generate_initial_data_onecat(
    ...
    X_gmm, z_plan = gmm_sampling(
        n_samples=n_samples,
        z=[22 / 50, 22 / 50, 6 / 50],
        mus=[mu_p1, mu_p2, mu_p3],
        covs=[z_p1, z_p2, z_p2],
    )
    # print("z_plan : ", z_plan)
    ...
    z_feature_cat_uniform, z_feature_cat_uniform_numeric = (
        generate_synthetic_features_multinomial(
            X=XZ_enc,
            index_informatives=[0, 1, 2, 3, 4, 5],
            list_modalities=["A", "B", "C"],
            list_beta=[
                np.array([2, -1, -3, -1, 1.5, 0]),
                np.array([2.1, -0.9, -3.1, 1, -1.5, 0]),
                np.array([1.9, -1.1, -2.9, 1, 1.5, 1.3]),
            ],
    ...
    print(
        "Plan a : ",
        Counter(z_feature_cat_uniform[(z_plan == "a") & (target_numeric == 1)]),
    )
    print(
        "Plan b : ",
        Counter(z_feature_cat_uniform[(z_plan == "b") & (target_numeric == 1)]),
    )
    print(
        "Plan c : ",
        Counter(z_feature_cat_uniform[(z_plan == "c") & (target_numeric == 1)]),
    )
