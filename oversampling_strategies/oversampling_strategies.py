...
class MGSY(BaseOverSampler):
    ...
    def _compute_mu_and_cov(
        self, X_minority: np.ndarray, neighbors_by_index: np.ndarray, dimension: int, num_samples
    ) -> Tuple[np.ndarray, np.ndarray]:
        all_neighbors = X_minority[neighbors_by_index.flatten()]
        all_neighbors_reshaped = all_neighbors.reshape(num_samples, self.K + 1, dimension)
        mus = np.mean(all_neighbors_reshaped, axis=1)
        centered_X = X_minority[neighbors_by_index.flatten()] - np.repeat(mus, self.K + 1, axis=0)
