    ...
    def _fit_resample(self, X, y=None, n_final_sample=None):
        ...
        if self.to_str:
            new_samples = np.zeros(
                (n_synthetic_sample, dimension), dtype=str
            )  # general case
        else:
            new_samples = np.zeros(
                (n_synthetic_sample, dimension)
            )  # case only  binary categorical features
        current_all_n = 0
        seed_generator = self.random_state
        while current_all_n < n_synthetic_sample:
            if self.generator_params is None:
                curr_X, curr_target, curr_target_num = self.generator(
                    n_samples=n_synthetic_sample,
                    dimension=dimension,
                    random_state=seed_generator,
                    verbose=-1,
                )
            else:
                curr_X, curr_target, curr_target_num = self.generator(
            ...
            if current_all_n + len(curr_minority_X) < n_synthetic_sample:
                new_samples[current_all_n : current_all_n + len(curr_minority_X), :] = (
                    curr_minority_X
                )
                current_all_n += len(curr_minority_X)
            else:
                new_samples[current_all_n:, :] = curr_minority_X[
                    : (n_synthetic_sample - current_all_n), :
                ]
...
class OracleTwoCat5:
    ...
    def predict(self, X, scaler, y=None):
        ...
        z_feature_cat_uniform, z_feature_cat_uniform_numeric = (
            generate_synthetic_features_multinomial_quadruple(
                X=inversed_X,
                index_informatives=[0, 1, 2],
                list_modalities=["Ae", "Bd", "Af", "Ce"],
        ...
        first_cat_feature = np.array(
            [z_feature_cat_uniform_numeric[i][0] for i in range(n_samples)]
        )
        second_cat_feature = np.array(
            [z_feature_cat_uniform_numeric[i][1] for i in range(n_samples)]
        )
        feature_cat_uniform_numeric_final = np.hstack(
            (first_cat_feature.reshape(-1, 1), second_cat_feature.reshape(-1, 1))
        )
...
class OracleTwoCat:
    ...
    def predict(self, X, scaler, y=None):
        ...
        feature_cat_uniform_numeric_final = np.hstack(
            (
                feature_cat_uniform_numeric.reshape(-1, 1),
                feature_cat_uniform_numeric2.reshape(-1, 1),
            )
        )
        return feature_cat_uniform_numeric_final
